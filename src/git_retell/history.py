...
def start(
    repo: Path, name: str, base: str, target: str, path: Path, budget: int
) -> History:
    validate_name(name)
        ...
        steps.append(
            {
                ...
                **churn(history.repo, previous, commit),
            }
        )
        previous = commit
...
def finish_report(
    history: History, limit: int, issues: list[str], steps: list[dict], previous: str
) -> dict:
    ...
    if tip_tree != target_tree:
        issues.append(
            "Final tree differs from the pinned target tree; history is unfinished."
        )
    ...
    return {
        ...
        "real_presentation_lines": line_count(
            diff(history.repo, history.base, history.target)
        ),
